...
async function seed() {
    ...
    const planRepo = app.get((0, typeorm_1.getRepositoryToken)(plans_entity_1.Plan));
    const policyRepo = app.get((0, typeorm_1.getRepositoryToken)(policy_entity_1.Policy));
    const claimRepo = app.get((0, typeorm_1.getRepositoryToken)(claim_entity_1.Claim));
    ...
    const policiesData = [
        { user: users[0], plan: plans[0], policyType: policy_entity_1.PolicyType.HEALTH, status: policy_entity_1.PolicyStatus.ACTIVE },
        { user: users[0], plan: plans[1], policyType: policy_entity_1.PolicyType.MOTOR, status: policy_entity_1.PolicyStatus.ACTIVE },
        { user: users[1], plan: plans[2], policyType: policy_entity_1.PolicyType.HOME, status: policy_entity_1.PolicyStatus.ACTIVE },
        { user: users[1], plan: plans[4], policyType: policy_entity_1.PolicyType.LIFE, status: policy_entity_1.PolicyStatus.ACTIVE },
        { user: users[2], plan: plans[3], policyType: policy_entity_1.PolicyType.TRAVEL, status: policy_entity_1.PolicyStatus.ACTIVE },
        { user: users[2], plan: plans[5], policyType: policy_entity_1.PolicyType.ACCIDENT, status: policy_entity_1.PolicyStatus.LAPSED },
    ];
    const policies = [];
    ...
    const claimsData = [
        { policy: policies[0], claimAmount: 15000, description: 'Hospital admission for fever treatment', status: claim_entity_1.ClaimStatus.APPROVED },
        { policy: policies[0], claimAmount: 5000, description: 'Medication expenses', status: claim_entity_1.ClaimStatus.PENDING },
        { policy: policies[1], claimAmount: 25000, description: 'Car accident repair', status: claim_entity_1.ClaimStatus.IN_REVIEW },
        { policy: policies[2], claimAmount: 50000, description: 'Water damage repair', status: claim_entity_1.ClaimStatus.PENDING },
        { policy: policies[3], claimAmount: 100000, description: 'Critical illness claim', status: claim_entity_1.ClaimStatus.REJECTED },
        { policy: policies[4], claimAmount: 10000, description: 'Lost baggage claim', status: claim_entity_1.ClaimStatus.APPROVED },
    ];
